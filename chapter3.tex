...
本章阐述本文所设计的面向扰动水下环境新视角合成系统。本文以4D Gaussian Splatting\upcite{wu2024gaussian}为动态场景表示骨架，将SeaSplat\upcite{seasplat2025icra}中验证有效的水下物理成像模型迁移至其渲染流程；同时借鉴UDR-GS\upcite{udrgs2024symmetry}将单目深度先验融入水下动态重建的思路，由于UDR-GS未开源，本文以Depth Anything V2\upcite{yang2024depthv2}为深度先验来源对该深度监督机制进行独立实现，并将其与水下物理模块组合，形成水下物理模型、动态高斯表示与深度监督相统一的框架。本章主要工作包括：（1）确定全文符号约定并给出整体管线（pipeline）的执行流程概述；（2）将SeaSplat的后向散射、衰减子网络与水下专用损失迁移至4DGS渲染流程中，并解决两套框架在接口、参数管理和动态深度上的差异；（3）以Depth Anything V2为深度源对UDR-GS的单目深度监督思路进行独立复现，并对尺度不确定性进行处理；（4）针对多组件联合优化的不稳定性，在4DGS自身的粗精两段训练之上，进一步在精细阶段内部细分出"水下网络预热，高斯颜色调整，联合优化"的子阶段流程，这是本文针对水下动态重建提出的分阶段训练策略。

\section{符号约定}
\label{sec:notation}

为便于全文统一书写，本节先给出符号约定。图像级张量用粗体表示：观测图像记为$\mathbf{I}$，与之对应的真实观测图像记为$\mathbf{I}^{\mathrm{gt}}$，物理分解后的干净颜色图记为$\mathbf{J}$，渲染深度图记为$\mathbf{Z}$。逐通道分量用下标$c\in\{r,g,b\}$表示，单点深度的标量值记为$Z$。水下物理成像模型中，$\beta_c^D$表示直接透射衰减系数，$\beta_c^B$表示后向散射衰减系数，$B_c^{\infty}$表示无穷远后向散射强度。高斯基元的可学习属性包括位置$\boldsymbol{\mu}\in\mathbb{R}^3$、协方差$\boldsymbol{\Sigma}\in\mathbb{R}^{3\times3}$、不透明度$\alpha\in(0,1)$、球谐系数$\mathbf{f}$以及时间戳$t$。物理建模公式以分量形式书写（如$I_c=J_c\exp(-\beta_c^D Z)+B_c^\infty(1-\exp(-\beta_c^B Z))$），张量实现按逐元素运算自然推广，本章后续公式均沿用上述记号。
...
\textbf{动态建模层。}4DGS通过时间条件化的变形网络（Deformation Network）对高斯点云的位置、旋转与尺度进行时序调制，结合HexPlane多分辨率特征平面表达动态场景。本文继承该机制以应对水下扰动引入的非刚性运动。
...
\textbf{（3）高斯光栅化。}经变形后的高斯点云通过可微分的基于分块排序的光栅化算法投影至图像平面，输出干净场景颜色图$\mathbf{J}$及深度图$\mathbf{Z}$（符号约定见\ref{sec:notation}节）。$\mathbf{J}$代表在无水体介质干扰时的真实辐射度。
...
本节给出本文从SeaSplat迁移并改写到4DGS渲染流程中的三个具体模块：后向散射网络（BackscatterNet）、直接透射衰减网络（AttenuateNet）以及四项水下专用损失函数。其中，参数初始化、参数组注册以及对4DGS动态深度输入的适配均为本文针对动态框架进行的改写工作。
...
\end{equation}网络结构为两层轻量卷积，参数量约1K，作为单独参数组纳入优化器。
...
\end{equation}参数以较小正值初始化，使系统初始阶段接近无衰减状态，便于粗训练阶段高斯几何稳定收敛。
...
上述过程整个合成路径完全可微，梯度可同时流向高斯点集参数和水下网络参数。

\section{单目深度监督实现}

\subsection{深度图生成流程}

在水下动态重建中引入单目深度监督的思路最早由UDR-GS\upcite{udrgs2024symmetry}提出，用于缓解水下几何重建的深度歧义性。由于UDR-GS未提供开源代码，本节以Depth Anything V2\upcite{yang2024depthv2}为深度先验来源对该机制进行独立实现，并与本文迁移得到的水下物理模块及第\ref{sec:training-stages}节中的分阶段训练策略相耦合。Depth Anything V2在多种室内外场景上具备较好的跨域泛化能力，其Large变体的单图推理时间在百毫秒量级，将其单目逆深度输出离线预处理后接入4DGS训练流程，能够满足实用需求。
...
分阶段训练策略用于降低多组件联合优化的不稳定性。当水下网络、变形网络与高斯点集同时从零开始优化时，颜色退化、几何误差和动态形变误差会在同一光度损失中相互干扰：水下网络若过早引入，可能把几何错位解释为后向散射或衰减；变形网络若在几何尚未稳定时充分更新，则容易把水体造成的颜色差异误当作运动信号。为此，本文不直接端到端联合启动全部模块，而是先优化几何结构，再优化介质损失，最后联合交替优化，并依次划分训练阶段。

需要说明的是，本文的训练流程在最外层继承了4DGS\upcite{wu2024gaussian}原有的"粗训练—精细训练"二段框架：粗训练阶段在不开启水下物理模型的条件下与原始4DGS等价，用于建立基础几何结构；精细训练阶段则启用水下模块、深度监督等本文新增的组件。本文的设计工作集中于精细训练阶段内部，针对水下网络、动态高斯属性与深度监督之间的梯度耦合，将其进一步细分为"后向散射/衰减预热—高斯颜色调整—联合训练"三个子阶段，并在联合训练阶段引入间歇式（burst）专项更新策略以维持优化动态平衡。完整的阶段划分及各阶段中的可学习参数与损失项配置如图\ref{fig:training_stages}所示。

阶段划分对应三类优化对象的启停边界：高斯几何与密度控制首先建立可渲染的基础结构；水下参数随后在固定几何上估计衰减和后向散射的量级；高斯颜色再在相对稳定的水下参数下校准到干净外观空间；最后进入全参数联合优化。

该策略本质上是训练稳定性设计，消融实验单独证明每个训练阶段的必要性。

\begin{figure}[htbp]
  \centering
  \input{figures/fig3-4-training-stages.tex}
  \caption{分阶段训练策略流程图。最外层的粗训练与精细训练两段框架继承自4DGS（蓝色虚线框），本文在精细训练阶段内部进一步细分出后向散射/衰减预热、高斯颜色调整与联合训练三个子阶段（橙色虚线框），并在联合训练阶段引入间歇式专项更新。}
